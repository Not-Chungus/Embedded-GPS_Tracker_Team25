...
int main(void) {
    char c;
    int pos = 0;  // Cursor position on current line
    int line = 0; // Current line (0 or 1)

    UART_init();
    lcd_init();

    lcd_disp_string("UART->LCD Ready");
    delayMs(1000);
    ...
    while (1) {
        if (UART_IsCharAvail()) {
            ...
            if (pos >= LCD_MAX_COLS) {
                pos = 0;
                line ^= 1; // Toggle between 0 and 1
                lcd_command(0x01); // Optionally clear display between lines
            }
        }
    }
}
